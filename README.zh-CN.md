...
这是一个恶搞玩具。所有生成内容都是机器编造的，没有一句出自任何真人之手。

## 功能

- **按时代还原风格** —— 风格样本按年份（及月份）分组，取自约 6000 条旧帖的公开存档。每一年的大小写风格和倾向都是从真实数据里统计出来的，所以 2013 年在咆哮，2018 年基本不咆哮。
- **自带 API Key（任意厂商）** —— 支持 Anthropic (Claude)、OpenAI、Google Gemini、DeepSeek、Kimi、千问、MiniMax，以及任何兼容 OpenAI 的接口。Key 只保存在你自己的浏览器里（localStorage），并直接发送给你选择的厂商。
- **免费离线模式** —— 没有 Key 会退回到本地生成：把该时代的真实帖子重新拼接。连贯性更低，混乱度更高。
- **高度还原的仿真卡片** —— 四种 UI 风格（早期 / 经典 / 现代 / 暗黑），按年份匹配头像和显示名，外加一组 Ye 风格字体（Helvetica、Times、Impact、等宽）。每张卡片都带 **Parody** 标识，防止被当成真的。
- **内容安全** —— 生成之前会拦截辱骂／歧视性输入（虽然这不符合 Ye 的风格）；对从原始存档里冒出来的脏词，输出时会打码。
